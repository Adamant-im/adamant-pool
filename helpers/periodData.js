...
const {dbTrans, dbBlocks, dbRewards} = require('./DB');
...
module.exports={
	...
	refresh:async function(){
		...
		const rewardsForPeriod = await dbRewards.syncFind({timeStamp:{$gte:startPeriod}});
		...
		const blocksPeriod=(await dbBlocks.syncFind({unixTimestamp:{$gte:startPeriod}})).sort((a,b)=>a.unixTimestamp-b.unixTimestamp);
		...
		console.log({usertotalreward, totalforged:this.forged});
	},
	zero(){
		this.forged=0;
