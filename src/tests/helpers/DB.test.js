import {dbTrans as db} from '../../helpers/DB.js';
...
describe('db.insert', () => {
  ...
  test('db.insert should insert 3 objects', async () => {
    await db.insert({}, {}, {});

    expect(db.data.values.length).toBe(3);
  });
...
describe('db.find', () => {
  ...
  test('db.find called with a string should throw error', () => {
    return expect(db.find('a string')).rejects.toThrow(
        'query should be a function or object, but got a string',
    );
  });

  test('db.find called with a number should throw error', () => {
    return expect(db.find(69)).rejects.toThrow(
        'query should be a function or object, but got a number',
    );
  });
});
...
describe('db.findOne', () => {
  ...
  test('db.findOne called with a string should throw error', () => {
    return expect(db.findOne('a string')).rejects.toThrow(
        'query should be a function or object, but got a string',
    );
  });

  test('db.findOne called with a number should throw error', () => {
    return expect(db.findOne(69)).rejects.toThrow(
        'query should be a function or object, but got a number',
    );
  });
});
