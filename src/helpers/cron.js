...
import {payOut} from '../modules/payOut.js';
...
export default {
  ...
  initCron(payoutperiod) {
    ...
    try {
      const payoutCronJob = new cron.CronJob(pattern, function() {
        payOut();
      });
...
function exit(additionalInfo) {
  log.error(
      `Pool's ${config.address} config is wrong. Failed to validate payoutperiod: ${config.payoutperiod}${additionalInfo ? ', ' + additionalInfo : ''}. Cannot start Pool.`,
  );
