...
module.exports = {
  ...
  async distributeRewards(block) {
    ...
    if (Store.isUpdatingVoters || Store.isDistributingRewards) {
      log.info(`Delegate is updating voters or distributing rewards for another block. To distribute rewards on block ${block.id} (height ${block.height}), I'll wait for ${Math.round(RETRY_WHEN_UPDATING_VOTERS_TIMEOUT / 1000)} seconds.`);
      setTimeout(() => {
    ...
    try {
      ...
      log.error(`Error in distributeRewards() on block ${block.id} (height ${block.height}): ` + e);
    }
